...
/**
 * 本程序用java来实现Email的发送，所用到的协议为：SMTP，端口号为25；
...
//		SendSimpleMail themail = new SendSimpleMail("smtp.qq.com");//这里以qq邮箱为例子
		SendSimpleMail themail = new SendSimpleMail("113.108.16.44");//这里以qq邮箱为例子
		String mailbody = new String(properties.getProperty("mailBody").getBytes("ISO8859-1"),"UTF-8");//邮件正文
		themail.setNeedAuth(true);
		themail.setSubject("JAVA发邮件的测试");//邮件主题
		themail.setBody(mailbody);//邮件正文
		
		//以下信息自行修改
		themail.setTo(new String[]{"*********@locatino.com"});//收件人地址
		themail.setFrom("*******@qq.com");//发件人地址
		themail.addFileAffix("F:/package.txt");// 附件文件路径,例如：C:/222.jpg,*注；"/"的写法； 如果没有可以不写
		themail.setNamePass("********@qq.com", "********");//发件人地址和密码 **改为相应邮箱密码
		themail.sendout();
	}
}
